...
def add_todo(migi:AI)->bool:
    item = Item()
    migi.say("Tell me what to add to the list")
    try:
...
def list_todos(migi:AI):
    if len(todo) > 0:
        migi.say("Here are your to do's")
        for item in todo:
            migi.say(item.title)
    else:
        migi.say("The to do list is empty!")

def remove_todo(migi:AI)->bool:
    migi.say("Tell me which item to remove")
    try:
